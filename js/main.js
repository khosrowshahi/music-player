// let's select required tags or elements with
...
showMoreBtn = wrapper.querySelector("#show-more"),
hideMoreBtn = musicList.querySelector("#hide-more");
...
function nextMusic() {
  ...
  musicIndex++;
  // if musicIndex is greater than 1 then musicIndex will be array length so the last song will play 
...
progressArea.addEventListener("click", (e)=>{
  ...
  let songDuration = mainAudio.duration; // getting duration of song (total)
...
repeatBtn.addEventListener("click", () => {
  ...
  switch(getText) {
    case "repeat": // if this icon is repeat then change it to repeat_one
      ...
      repeatBtn.setAttribute("title", "repeat once")
      break;
...
mainAudio.addEventListener("ended", () => {
  // we'll do according to the icon means if user has set icon to loop song then
  // we'll repeat the current song and will do further accordingly

  let getText = repeatBtn.innerText; // getting innerText of the icon
  switch(getText) {
    case "repeat": // if this icon is repeat then simply we can call the nextMusic function so the next song will play
      nextMusic();
      break;
...
for(let i = 0; i < allMusic.length; i++) {
  ...
  liAudioTag.addEventListener("loadeddata", () => {
    ...
    // adding t duration attribute wich we'll use below
...
function playingNow() {
  for(let j = 0; j < allLiTags.length; j++) {
    ...
    if(allLiTags[j].classList.contains("playing")) {
      ...
      audioTag.innerText = adDuration; // passing t-duration value to audo=io duration innerText
    }

    // if there is tag which li-index is equal to musicIndex,
...
// const ulTag = wrapper.querySelector("#dynamicUl");
...
//                   <p>${allMusic[i].artist}</p>
//                 </div>
//                 <audio class="allMusic" src="songs/${allMusic[i].src}.mp3"></audio>
//                 <span id="allMusicDuration" class="audio-duration"></span>
//                </li>`;
//   ulTag.insertAdjacentHTML("beforeend", liTag);
// }

// let liAudioDuration = ulTag.querySelectorAll('#allMusicDuration');
// let liAudioTag = ulTag.querySelectorAll('.allMusic');

// liAudioTag.forEach((audio, idx) => {
...
//       totalSec = `0${totalSec}`;
//     }
//     liAudioDuration[idx].innerText = `${totalMin}:${totalSec}`;
//   });
// })
